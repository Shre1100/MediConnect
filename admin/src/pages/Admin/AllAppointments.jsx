        ...
        <div className="p-5 bg-white w-full mr-10 rounded-xl flex flex-col gap-5">
            ...
            <div className="flex flex-col max-h-[80vh] overflow-y-scroll">
                ...
                {appointments.map((item,idx)=>(
                    <div className="flex flex-wrap items-center justify-between max-sm:gap-2 sm:grid grid-cols-[0.5fr_3fr_1fr_3fr_3fr_1fr_1fr] grid-flow-col py-3 px-6 border-b" key={idx}>
                        <p className="max-sm:hidden">{idx+1}</p>
                        <div className="flex items-center gap-3">
                            <img className="w-10 rounded-full" src={item.userData.image} alt="Patient Image" />
                            <p>{item.userData.name}</p>
                        </div>
                        <p className="max-sm:hidden">{calcAge(item.userData.DOB)}</p>
                        <p>{slotDateFormat(item.slotDate)} at {item.slotTime}</p>
                        <div className="flex items-center gap-3">
                            <img className="w-10 rounded-full" src={item.docData.image} alt="Patient Image" />
                            <p>{item.docData.name}</p>

                        </div>
                        <p>{currency}{item.amount}</p>
                        {item.cancelled ?
                        <p className="px-2 py-1 bg-gray-200 rounded-2xl cursor-not-allowed">Cancelled</p>:
                        <button onClick={()=>cancelAppointments(item._id)} className="px-4 py-1 bg-red-400 rounded-2xl cursor-pointer hover:bg-red-600 text-white font-semibold">Cancel</button>
                        }
