        ...
        <div id="speciality" className="flex flex-col items-center gap-3 py-14 text-slate-600">
            ...
            <div className="flex sm:justify-center gap-3 pt-5 w-full overflow-scroll">
                {specialityData.map((item,index)=>(
                    <Link onClick={()=>scrollTo(0,0)} className="flex flex-col items-center cursor-pointer flex-shrink-0 text-sm hover:translate-y-[-7px] transition-all duration-500 hover:drop-shadow-md" key={index} to={`/doctors/${item.speciality}`}>
                        <img className="w-[4.5rem] sm-w-24 mb-2" src={item.image} alt="speciality image" />
                        <p>{item.speciality}</p>
                    </Link>
                ))}
            </div>
